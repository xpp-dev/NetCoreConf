...
namespace NetCoreConfFunctionApp
{
    public static class ImportPackage
    {
        [FunctionName("ImportPackage")]
        public static async Task Run([BlobTrigger("packages/{name}.zip", Connection = "AzureWebJobsStorage")]Stream blob, string name, ILogger log)
        {
            try
            {
                var token = GetToken(
